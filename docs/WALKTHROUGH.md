...
**Was tut es?**
Read-only `GET /api/v1/worker_notes/search` (`q`/`machine_id`/`k`), auth-pflichtig. **Ab Paket 1a hybrid:** deutscher Volltext (`text_tsv`/GIN, Migration 0012) + Vektor, per RRF (k=60) fusioniert, mit Relevanz-Cutoff (`FOREMAN_ARCHIVE_VECTOR_MAX_DISTANCE`, Default 0.60 — gemessen, C-048). `api/deps.py` liefert `EmbeddingProviderDep` + `SettingsDep`.
...
**Was tut es?**
Ein NEUER, additiver Endpoint `GET /api/v1/archive/search` (`q`/`machine_id`/`sources`/`k`) durchsucht VIER Quellen zugleich: Notizen (der 1a-Hybrid, unverändert wiederverwendet), Wartungsprotokolle und Alarme (je reiner deutscher Volltext über `text_tsv`/GIN, Migration 0013 — Freitext primär, Code ergänzend) und das Gedächtnis (semantischer Abruf gegen die Gegenstelle, best-effort, §15.10). Jede Quelle liefert eine Rangliste; `_fusioniere` bildet sie auf einen gemeinsamen VORGANGSSCHLÜSSEL ab und summiert `1/(60 + Rang)` über die findenden Quellen — Einigkeit hebt den Treffer, statt verworfen zu werden. Ausgabelänge 15 (`ARCHIV_AUSGABELAENGE`, gemessen — C-087). Antwort: flache `list[ArchiveHit]` (Quelle, id, Maschine, normalisierter Zeitstempel, an der Wortgrenze gekürzter Auszug, PII-freie Anzeige-Details, `gefunden_von`) OHNE Score. `source_type` sagt, WAS ein Treffer ist; `gefunden_von`, WER ihn fand. Über `sources` (wiederholbar oder CSV) lassen sich Quellen aus-/einblenden (Default alle).
