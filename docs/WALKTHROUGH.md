...
### Eigenprofil-Overlay — persistiert (`reasoners/drift/service.py`, `reads/trend.py`, Migration `0011`)

**Was tut es?**
Das Drift-Eigenprofil — der gleitende Median je Betriebszustand plus die robuste Rausch-Streuung, gegen die der Detektor jede Maschine bewertet — wird am Laufende je Datenpunkt persistiert (`drift_profiles`). Die Read-Schicht expandiert es je Trend-Punkt zum Erwartungskorridor `median(zustand) +/- k*streuung` und liefert ihn über `profile_band` an das HTTP-Erstbild **und** den WS-Snapshot; das Dashboard (Sektion B) zeichnet ihn als gestrichelten Korridor neben dem statischen Normalband, mit sichtbarem Profil-Stand.

**Warum existiert es / wo sitzt es?**
Ehrlichkeitslinie (wie der Sim-Vorbehalt bei F-PRED, die abgeleitete Topologie bei I): das gezeigte Band ist nachweislich die **echte Bewertungsbasis des Detektors** — derselbe Zustands-Median, dieselbe Streuung, kein im Read neu erfundener Wert — und die Zustands-Zuordnung (Tagesstunde) nutzt **dieselbe** Funktion wie der Detektor-Lauf, sonst zeigte das Band den Korridor des falschen Zustands. Kein etabliertes/ausreichendes Profil → `profile_band` bleibt null, das Frontend lässt das Overlay weg (kein geratenes Band); DB-CHECKs (`noise_sigma > 0`) halten diese Linie auch an der Persistenzgrenze. Der Profil-Stand (`computed_at`) wird ehrlich als „Stand" gezeigt, nicht als Live-Wert.
...
**Was tut es?**
Read-only `GET /api/v1/worker_notes/search` (`q`/`machine_id`/`k`), auth-pflichtig. **Ab Paket 1a hybrid:** deutscher Volltext (`text_tsv`/GIN, Migration 0012) + Vektor, per RRF (k=60) fusioniert, mit Relevanz-Cutoff (`FOREMAN_ARCHIVE_VECTOR_MAX_DISTANCE`, Default 0.55). `api/deps.py` liefert `EmbeddingProviderDep` + `SettingsDep`.

**Warum existiert es / wo sitzt es?**
**Vor** dem worker_notes-CRUD gemountet (sonst fängt `/{note_id}` den `/search`-Pfad). **Graceful (Paket 1a):** ein Embedding-Backend-Ausfall ergibt KEIN 503 mehr — der Volltext-Zweig trägt allein (200; leere Liste nur, wenn auch der Volltext nichts findet). So findet „Fett" seine Notiz über den Wortlaut, und der Cutoff hält Unzusammenhängendes draußen. Die reine Vektor-Komposition `embed_and_search` (F6-Anbindung) reicht ihren `EmbeddingError` weiter ehrlich durch (dort best-effort gefangen).

### Archiv-Quellen — Wartung + Alarme (`archive/`, Paket 1b)

**Was tut es?**
Ein NEUER, additiver Endpoint `GET /api/v1/archive/search` (`q`/`machine_id`/`sources`/`k`) durchsucht drei Quellen zugleich: Notizen (der 1a-Hybrid, unverändert wiederverwendet), Wartungsprotokolle und Alarme (je reiner deutscher Volltext über `text_tsv`/GIN, Migration 0013 — Freitext primär, Code ergänzend). Jede Quelle liefert eine Rangliste; per RRF (k=60) zu einem globalen Rang fusioniert. Antwort: flache `list[ArchiveHit]` (Quelle, id, Maschine, normalisierter Zeitstempel, an der Wortgrenze gekürzter Auszug, PII-freie Anzeige-Details) OHNE Score. Über `sources` (wiederholbar oder CSV) lassen sich Quellen aus-/einblenden (Default alle).

**Warum existiert es / wo sitzt es?**
Eigenes Subsystem `src/foreman/archive/` (wie `audit/`/`topology/`); der Notiz-Zweig reuse't den 1a-Hybrid, `notes/` bleibt unberührt. Der alte `/worker_notes/search` (1a) bleibt unverändert — die FE-Ablösung kommt erst in Paket 1c. Wartung/Alarm tragen kein Embedding, darum reiner Volltext (kein Vektor/kein Cutoff — ein Treffer IST ein Volltext-Match). Graceful wie 1a (Embedding-Ausfall → Notiz-Volltext trägt, Wartung/Alarm unberührt, kein 503). **Datenschutz-Befund (gemeldet, nicht gelöst):** `maintenance_events.description` und `alarms.message` werden im Schreibpfad NICHT NER-maskiert (anders als der Notiz-Text) — sie werden so ausgeliefert, wie gespeichert; die Vereinheitlichung gehört in Paket 2.
...
- `components/machine/TimeSeriesChart` ist ein maßgeschneidertes, token-getriebenes SVG (bewusst KEINE Charting-Lib — hält das <100-kB-Erstbild-Ziel, volle Kontrolle über die Kodierung): Normalband als entsättigte Fläche, Drift als Differenzfläche (`diff-over`/`diff-under` + Schraffur-Pattern), F4-Eigenprofil-Korridor (persistiertes `profile_band` → gestrichelter Erwartungskorridor in `data-series-2` mit Profil-Stand am Panel; null → graceful weggelassen), mehrkanalig (Linie + Fläche + Schraffur + aria-Label), Drift als Akzent — nie Alarm-Rot. Dazu `MachineHeader` (Identität + FCSM groß live über `machine:{id}` + Schnellaktionen), `MachineSpecs`, `MachineHistory` (PII maskiert), `MachineAlarms` (bettet die C-`AlarmRow` ein, maschinengefiltert — kein dupliziertes Rendering), `MachineList`, `MachineCrossLinks`, `SensorPicker`/`TimeWindowPicker`, `MachineDetailView` (Orchestrator, Rollen-Split ohne bedingte Hooks).
...
### Kanonische lebende Maschinenkarte (`reads/card.py`, `reads/datapoint_status.py`, `frontend/lib/machine/card.ts`+`grouping.ts`, `frontend/components/machine/machine-card*.tsx`)

EINE „lebende Maschinenkarte" als EINE FE-Komponente ersetzt die bisherigen Maschinenlisten-Reiter (`/machines`-Grid) UND die tote Stammdaten-Box (`machine-specs`, zeigte nur Name·Einheit). Pro Datenpunkt jetzt der **aktuelle Wert + Status**, nicht nur die Einheit. Voller Vertrag: GROUND_TRUTH §21.19 (+ §4 Routen, §20.1 Read-Core).

**Was ist es / wo sitzt es?**
- Backend Read-Core: `reads/card.py` baut die `MachineCard` (Steckbrief + Komponenten + Datenpunkte-mit-Wert/-Status + Maschinen-Status + Stream) — `build_fleet_cards` (Grid, batched, kein N+1) und `build_machine_card` (= Scope `[id]`, Detail + WS). Der jüngste Wert je Datenpunkt kommt aus `reads/queries.latest_values_for_data_points` (`DISTINCT ON … ORDER BY bucket DESC` über `readings_1m`). `reads/datapoint_status.derive_datapoint_status` leitet den Status NUR aus bestehenden Signalen ab (Alarm-Verdikt → Eigenprofil-Korridor `corridor_at` → statisches Normalband → `unknown`); `corridor_at` (`reasoners/drift/baseline.py`) ist die geteilte Detektor-Band-Quelle, jetzt auch vom Trend-Overlay genutzt (DRY). Routen `GET /api/v1/cards` (scope-gefiltert je Rolle) + `GET /api/v1/machines/{id}/card` (scope-gated); der WS-`machine:{id}`-Snapshot trägt die ganze `MachineCardOut`. NOTIFY-Anreicherung (`reads/queries.machines_for_data_points` in `ingestion/service` + `POST /readings`): ein Readings-Tick frischt auch `machine:{id}` + `overview` auf → die Karte lebt pro Tick.
- Frontend: `lib/machine/card.ts` (Wertformat deutsch, Status → Hallensprache-Ansicht Verdikt/Beobachtung, ehrliche Stale-Anzeige „Stand vor X"), `lib/machine/grouping.ts` (Klasse → Synoptik-Stufe Fördern/Pressen/Handling/Bestücken/Endkontrolle, kanonische Reihenfolge). `components/machine/MachineCard` ist die EINE Komponente in zwei Dichten (`compact` Grid / `full` Detail-Stammdaten), live über `useTopicState<MachineCardOut>(machine:{id})` mit SSR-Erstbild; `MachineCardGrid` gruppiert nach Stufe. `/machines` zieht `GET /cards`, `/machines/[id]` zieht EINE Quelle `GET /machines/{id}/card` und rendert `MachineCard full` statt `MachineSpecs`.

**Warum so?**
- EINE Quelle der Wahrheit (Komponente + Vertrag), kein Doppel-Code: dieselbe `MachineCard` trägt Grid und Detail; `MachineList`/`MachineSpecs` entfernt (der Ersatz macht sie obsolet). Ehrlichkeitslinie: der per-Datenpunkt-Status erfindet KEINEN Schwellwert — er nutzt die echte Alarm-/Detektor-Band-Bewertung; `unknown` bleibt unbekannt; bei Stream-Stopp wird der Stand benannt statt Frische vorgetäuscht. Hallensprache im sichtbaren UI (kein „Drift"/internes Vokabular). Synoptik-Vorbau: die Karten SIND die spätere Synoptik-Darstellung.
...
- **Backend-Stand (Paket 1a, Audit):** die Suche, auf der H aufsitzt, ist seit Paket 1a **Archiv-Backend hybrid** — deutscher Volltext + Vektor, per RRF fusioniert, mit Relevanz-Cutoff, graceful auf Volltext statt 503 (GROUND_TRUTH §15.8). Der FE-Vertrag ist **unverändert** (flache Liste, Position = Rang, kein Score) → diese Sektion läuft ohne Änderung weiter; nur die ehrlich benannte „503"-Anzeige greift für den Embedding-Ausfall nicht mehr (degradiert jetzt auf Volltext). **Noch offen (Paket 1c):** die FE-Trennung in zwei ehrliche Räume — „Archiv" (Stichwort-/Wortlaut-Suche) vs. „Hatten wir das schon mal" (intelligente Verknüpfung). H bündelt heute beides unter `/memory`; die Trennung ist bewusst noch nicht gebaut. **Paket 1b** hat das Archiv-Backend zusätzlich um zwei Quellen erweitert (Wartung + Alarme) hinter dem neuen `/archive/search` (`ArchiveHit`, GROUND_TRUTH §15.9); die FE adoptiert diesen Endpoint samt Quelltyp-Anzeige ebenfalls erst in 1c.
