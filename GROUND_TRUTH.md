...
- **Aufruf-Vertrag pro Backend** (`LiteLLMBackend`, gesetzt in `from_settings`): Backends unterscheiden sich darin, welche Parameter sie annehmen — die Auswahl passiert im Backend, **nicht** über LiteLLMs `drop_params` (LiteLLM führt `temperature` für Anthropic als unterstützt und würde sie durchreichen).
  - **Lokal (Ollama/vLLM):** `temperature` (Default `0.0`) wird gesendet — Determinismus über Sampling.
  - **Cloud (Anthropic, aktuelle Generation):** `supports_sampling_params=False` → **keine** `temperature`/`top_p`/`top_k` (sie werden mit **HTTP 400** abgelehnt); Determinismus kommt dort aus dem Prompt. Zusätzlich `reasoning_effort="none"` → **Denken aus**: es ist per Default an und teilt sich das Ausgabe-Budget mit dem Antworttext; bei `max_tokens=None` setzt LiteLLM für Anthropic ersatzweise 4096, sonst drohen abgeschnittene Erklärungen. Denken einzuschalten ist eine eigene Entscheidung **mit** bewusst gesetztem `FOREMAN_LLM_MAX_TOKENS`.
