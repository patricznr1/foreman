...
- **Gates** (lokal grün): tsc strict 0, ESLint 0, Vitest 520 gesamt (91 neu für A: `lib/cockpit` + `components/cockpit`; der abgelöste Durchstich-Test entfällt), tokens:check synchron, `next build` ok (`/overview` ~116 kB First Load — bespoke SVG ohne Charting-Lib). Hidden-Term-Scan sauber (sichtbares Wording paraphrasiert „Drift" → „Abweichung"; `DriftHeatmap` ist nur interner Code-Name).
