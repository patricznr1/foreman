...
| B Maschinen-Detail | `/machines` · `/machines/[id]` | [KERN] | ✅ voll: bespoke SVG-TimeSeriesChart (Normalband-Fläche, Drift-Akzent, F4-Eigenprofil-Korridor), Kopf/Specs/Historie (PII)/Alarme (C-`AlarmRow`), Rollen-Varianten (§21.11) |
...
- **Umsetzung (FE-only):** die sektions-eigenen `lib/{alarms,event-chains,prediction,memory}/roles.ts` setzen die `manager`-Variante auf voll (`aggregateOnly=false`, `canTrigger`/`canAcknowledge`/`canDecide`/`jumpToDiagnosis=true`). Die View-Orchestratoren bekommen einen Manager-Voll-Zweig (`ManagerAlarmsView`, `ManagerPredictionView`; D fällt automatisch in `ChainsSingle`). Das **Aggregat bleibt als Überblicks-Kopf** (C: `AlarmSituationHeader`, E: `PredictionAggregate`); `ChainsAggregate` wird von keiner Rolle mehr gerendert (belassen).
- **Backend unverändert:** verifiziert, dass die Schreib-/Trigger-Routen (`…/drift/alarms/{id}/acknowledge`, `…/event_chain/reconstruct`, `…/failure/predict` + `…/recommendation`) **keine Rolle** prüfen (nur `CurrentUser`) → der manager-Vollzugriff erzeugt **keine toten Klicks/403**. `ACCESS_MATRIX` (Sektions-Zugang, `lib/auth/roles.ts`) bleibt ebenfalls unverändert; nur die **Intra-Sektions-Varianten** wurden aufgeweitet. Reale Quittier-Route nur für Drift; Nicht-Drift-Quittierung bleibt sichtbar deaktiviert-mit-Grund; E-Verwerfen ist client-seitig.
