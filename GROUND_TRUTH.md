...
- **Defense-in-Depth (§16.1, analog `failure_predictions`):** DB-CHECK-Constraints erzwingen `validation_status='simulation_only'`, `data_regime='simulation'`, `decision IN ('elevated_risk','normal')` und — als zweite Linie für Invariante II — `validation_caveat LIKE '%simuliert%'` (marker-basiert: der Vorbehalts-Text muss den Sim-Marker tragen, eine Umdeutung wird an der Persistenzgrenze abgewiesen; die App-Garantie bleibt der Schema-Validator). Die Empfehlung ist ein **diskretes Ereignis** → zusätzlich als `semantic_event` (`event_type=failure_recommendation`, **`data_regime=simulation` im Payload**) ans Substrat gespiegelt (§12.4), damit das Gedächtnis die Sim-Empfehlung nie als reale Prognose ablegt. Indizes: `ix_failure_recommendations_prediction`, `ix_failure_recommendations_machine_created`.
