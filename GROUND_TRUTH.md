...
- **Warum ein Schalter:** Der Treffer trägt `source_type="memory"` — einen vierten Wert, den die Anzeige kennen muss. Eingeschaltet wird erst, wenn beide Seiten ihn kennen.
  - **Berichtigt am 24.08.2026:** Hier stand, ein unbekannter Wert breche „die Zod-Prüfung der Antwort". Das trifft nicht zu — im Frontend gibt es kein Zod (`grep` über `frontend/` ohne Treffer, Kontrollmuster belegt den Lauf); die Antwort wird als TypeScript-Schnittstelle typisiert, was zur Laufzeit nichts prüft. Der tatsächliche Grund ist ein anderer und schwächer: Ein unbekannter Quelltyp fiele in der Anzeige durch die Label- und Glyph-Tabellen und erschiene ohne Beschriftung. Kein Absturz, aber ein Treffer, dessen Herkunft der Werker nicht lesen kann.
...
- **OFFEN — am 24.08.2026 gegen den Code neu erhoben.** Die frühere Fassung dieser Zeile führte `SourceType`, `SOURCE_LABEL` und `SOURCE_GLYPH` als offen und nannte einen „Zod-Contract". Das war überholt beziehungsweise falsch: Die drei Ersten sind gesetzt (`frontend/lib/api/contracts.ts:395` führt `"memory"` in der Union, `frontend/lib/memory/source.ts` das Label „Aus dem Gedächtnis" und das Kürzel `G`), einen Zod-Contract gibt es im Frontend nicht. Tatsächlich offen sind vier Punkte:
...
- **OWASP Web Top 10 (2025) — A01 Zugriffskontrolle:** drei Striche, alle serverseitig durchgesetzt und getestet. (1) **Keine Selbstregistrierung** — Konten und Rollen entstehen ausschließlich über die Betreiber-CLI, nicht über HTTP (§4). (2) **RBAC auf den Trigger-/Schreibrouten** — `require_roles` (§21.18), unzulässige Rolle → 403. (3) **Identitätsbindung der Schreibpfade** — der Personenbezug stammt aus dem Token, nicht aus dem Request-Body (§4/§8). Verifiziert: `tests/integration/test_auth.py`, `test_user_provisioning.py`, `test_authorship_binding.py`, `test_reasoner_rbac.py`. *Bewusst ehrlich: hier steht nur, was tatsächlich geprüft ist — die übrigen Web-Top-10-Positionen sind Threat-Model-Rahmen (oben), kein Coverage-Nachweis.*
...
- **Und `tools/pruefe_registerzahlen.py`** — `claims check` prüft die Form, dieses Werkzeug die Frische: Es hält die im Register geführten Kennzahlen gegen den heutigen Bestand und nennt jede, die seit ihrer Messung gewandert ist.
  ...
  **Warum ein eigenes Werkzeug und warum kein CI-Gate** (entschieden 24.08.2026, nach dem zweiten Fall derselben Fehlerklasse): Zwischen dem 10.08. und dem 24.08.2026 liefen drei Änderungen durch, ohne dass eine einzige Registerzahl nachgezogen wurde — die Regel „wer misst, trägt ein" stand da und griff nicht. Eine Regel, die zweimal reisst, braucht einen Träger und kein weiteres Merkblatt. Ein blockierender CI-Schritt wäre aber der falsche: Die Testzahl wandert mit jedem hinzugefügten Test, und jeden Beitrag aufzuhalten, um einen Stichtagswert nachzuziehen, wäre Bürokratie statt Sorgfalt. Der Ort, an dem die Frische zählt, ist der Übergang nach draussen — dort läuft es, zusammen mit `claims check`.
