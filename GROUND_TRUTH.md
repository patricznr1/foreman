...
- **OFFEN — am 24.08.2026 gegen den Code neu erhoben.** Die frühere Fassung dieser Zeile führte `SourceType`, `SOURCE_LABEL` und `SOURCE_GLYPH` als offen und nannte einen „Zod-Contract". Das war überholt beziehungsweise falsch: Die drei Ersten sind gesetzt (`frontend/lib/api/contracts.ts:395` führt `"memory"` in der Union, `frontend/lib/memory/source.ts` das Label „Aus dem Gedächtnis" und das Kürzel `G`), einen Zod-Contract gibt es im Frontend nicht. Tatsächlich offen sind vier Punkte:
  1. **Die vierte Quelle wird nie angefordert.** `frontend/lib/memory/use-memory-search.ts:31` führt `ALL_SOURCES = ["note", "maintenance", "alarm"]`, und Zeile 115 setzt den Parameter **immer** — entweder aus dem Filter oder aus dieser Liste. Das Backend wird damit auf drei Quellen eingegrenzt. **Solange das so ist, wäre auch der eingeschaltete Schalter wirkungslos** — das ist der eigentliche Blocker, nicht die Typ-Union.
  2. **Der Quellen-Umschalter kennt drei Werte** (`frontend/components/memory/memory-search-bar.tsx:21`, `:49`, `:62`). Eine vierte Quelle wäre nicht abwählbar.
  3. **Die Trefferkarte zeigt die Herkunft nicht.** `frontend/components/memory/archive-result-card.tsx` wertet `detail.type` (Wartung) und `detail.severity` (Alarm) aus, aber keines der drei Erinnerungs-Felder (`herkunft`, `erinnerung`, `maschinenklasse`), die `contracts.ts:408-413` bereits führt.
  4. **Der Listen-Schlüssel kollidiert.** `frontend/components/memory/memory-result-list.tsx:69` bildet ihn als `${hit.source}-${hit.id}`; jede Erinnerung trägt `id=0` (`src/foreman/archive/search.py:207`, bewusst — eine Erinnerung hat keinen Primärschlüssel). Alle Erinnerungstreffer bekämen denselben Schlüssel.
