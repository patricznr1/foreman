...
async def _data_point(c: AsyncClient) -> int:
    ...
    machine = await c.post("/api/v1/machines", json={"label": "M", "line_id": line.json()["id"]})
    dp = await c.post(
        "/api/v1/data_points",
...
def _batch(dp_id: int) -> dict[str, object]:
    return {
        "readings": [
            ...
            {
                "data_point_id": dp_id,
                "time": "2026-06-13T10:00:01+00:00",
                "value": 21.7,
                "quality": 0,
            },
...
async def test_batch_writes_to_hypertable(
    ...
    count = await raw_conn.fetchval("SELECT count(*) FROM readings WHERE data_point_id = $1", dp_id)
    assert count == 3
...
async def test_unknown_data_point_400(auth_client: AsyncClient) -> None:
    response = await auth_client.post(
        ...
        json={
            "readings": [
                {"data_point_id": 999999, "time": "2026-06-13T10:00:00+00:00", "value": 1.0}
            ]
        },
    )
...
async def test_readings_requires_auth(client: AsyncClient) -> None:
    response = await client.post(
        "/api/v1/readings",
        json={
            "readings": [{"data_point_id": 1, "time": "2026-06-13T10:00:00+00:00", "value": 1.0}]
        },
    )
