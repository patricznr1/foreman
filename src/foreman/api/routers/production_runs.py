...
#  Linien-Scope (§20.4): jede Route führt `ResourceScopeDep` — der Produktions-
#         kontext folgt der Sichtbarkeit seiner LINIE, nicht einer Maschine.
# ============================================================
...
async def create_production_run(
    body: ProductionRunCreate, session: SessionDep, scope: ResourceScopeDep
) -> ProductionRun:
    await scope.require_line(body.line_id)
...
async def list_production_runs(
    ...
    """Produktionsläufe im Linien-Ausschnitt des Anfragenden."""
    stmt = await scope.limit_to_lines(
        select(ProductionRun).order_by(ProductionRun.id), ProductionRun.line_id, line_id=line_id
    )
...
async def get_production_run(
    run_id: int, session: SessionDep, scope: ResourceScopeDep
) -> ProductionRun:
    """Ein Produktionslauf — 404 auch für eine Linie außerhalb des Ausschnitts."""
    obj = await session.get(ProductionRun, run_id)
    if obj is None or not await scope.can_see_line(obj.line_id):
        raise HTTPException(
